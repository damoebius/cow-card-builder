    ...
    public function fromWorkbook(workbook:Workbook):Void{
        ...
        for(i in 0...workbook.SheetNames.length){
            ...
            for(field in fields){
                ...
                switch(letter){
                    case 'A':
                        card.name = Reflect.field(sheet,field);
                    case 'B':
                        card.cost = Reflect.field(sheet,field);
                    case 'C':
                        card.type = Reflect.field(sheet,field);
                    case 'D':
                        card.att = Reflect.field(sheet,field);
                    case 'E':
                        card.def = Reflect.field(sheet,field);
                    case 'F':
                        card.description = Reflect.field(sheet,field);
                    default :
